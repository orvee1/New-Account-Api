<x-app-layout>
  <div class="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8 py-6 space-y-5">
    ...
    {{-- Context header --}}
    @php($ctx = $session ?? null)
    <div class="flex flex-wrap items-center gap-2">
      <a href="{{ $session ? route('batches.session', $session) : route('batches.index') }}"
        class="rounded-md bg-emerald-600 px-4 py-2 text-sm font-medium text-white hover:bg-emerald-700">← Back</a>

      <h1 class="text-xl font-semibold text-gray-900">Create Batch</h1>
    ...
    <form method="POST"
      action="{{ $session ? route('batches.session.store', $session) : route('batches.store') }}"
      class="space-y-6">
         @csrf

        @unless($session)
        {{-- generic store হলে id পাঠাতে হবে --}}
        <input type="hidden" name="course_session_id" value="{{ request('course_session_id') }}">
        @endunless
            ...
            <div class="flex items-center justify-between rounded-lg border border-gray-200 bg-gray-50 px-3 py-2">
              ...
              <span class="relative inline-flex items-center">
                ...
                <input id="is_show_admission" name="is_show_admission" type="checkbox" value="1"
                       {{ old('is_show_admission', true) ? 'checked' : '' }}
        ...
        <div class="p-5 grid grid-cols-1 gap-5">
          <div>
            ...
            <textarea name="description" rows="3"
              class="w-full rounded-md border-gray-300 focus:border-indigo-600 focus:ring-indigo-600">{{ old('description') }}</textarea>
          </div>

          <div>
            <label class="mb-1.5 block text-sm font-medium text-gray-700">Course Outline</label>
            <textarea name="course_outline" rows="3"
              class="w-full rounded-md border-gray-300 focus:border-indigo-600 focus:ring-indigo-600">{{ old('course_outline') }}</textarea>
          </div>

          <div>
            <label class="mb-1.5 block text-sm font-medium text-gray-700">Course Fee Offer</label>
            <textarea name="course_fee_offer" rows="3"
              class="w-full rounded-md border-gray-300 focus:border-indigo-600 focus:ring-indigo-600">{{ old('course_fee_offer') }}</textarea>
          </div>
          ...
          <div>
            ...
            <textarea name="registration_process" rows="3"
              class="w-full rounded-md border-gray-300 focus:border-indigo-600 focus:ring-indigo-600">{{ old('registration_process') }}</textarea>
          </div>

          <div>
            <label class="mb-1.5 block text-sm font-medium text-gray-700">Expired Message</label>
            <textarea name="expired_message" rows="2"
              class="w-full rounded-md border-gray-300 focus:border-indigo-600 focus:ring-indigo-600">{{ old('expired_message') }}</textarea>
          </div>
        </div>
      </div>
