...
<div class="max-w-4xl mx-auto px-4 py-6">
  <div class="flex items-center justify-between mb-6">
    ...
    <div class="flex items-center gap-3">
      <a href="{{ route('admin.companies.index') }}" class="text-sm text-gray-600 hover:underline">← Back</a>
    ...
    <div class="flex items-start gap-6">
      ...
      <div class="flex-1">
        ...
        <div class="mt-2 flex flex-wrap gap-2">
          ...
          @if($company->website)
            <a href="{{ $company->website }}" target="_blank" class="text-sm text-indigo-600 hover:underline">{{ $company->website }}</a>
          @endif
        </div>
      </div>
    </div>
    ...
    <dl class="mt-6 grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4">
      ...
      <div class="md:col-span-2">
        ...
        <dd class="text-sm font-medium text-gray-900">
          {{ $company->created_at?->format('Y-m-d H:i') }} @if($company->creator) by {{ $company->creator->name }} @endif
        </dd>
      </div>
      <div class="md:col-span-2">
        <dt class="text-sm text-gray-500">Last Updated</dt>
        <dd class="text-sm font-medium text-gray-900">
          {{ $company->updated_at?->format('Y-m-d H:i') }} @if($company->updater) by {{ $company->updater->name }} @endif
        </dd>
      </div>
    </dl>
  </div>
