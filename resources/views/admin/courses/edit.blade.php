    ...
    <div class="max-w-3xl mx-auto p-6">
        ...
        <form action="{{ route('courses.update', $course->id) }}" method="POST"
            ...
            <div class="grid grid-cols-1 gap-4">
                ...
                <div>
                    <label for="institute_id" class="block text-sm font-medium text-gray-700">Institute <span
                            class="text-red-500">*</span></label>
                    @php $selectedInstitute = $course->institute_id ?? ''; @endphp
                    <select id="institute_id" name="institute_id" required
                        class="mt-1 w-full rounded-md border-gray-300  focus:ring-indigo-500 focus:border-indigo-500">
                        <option value="">Select...</option>
                        @foreach ($institutes as $inst)
                        <option value="{{ $inst->id }}" @selected($selectedInstitute==$inst->id)>
                            {{ $inst->name }}
                        </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="text" id="bkash_merchant_number" name="bkash_merchant_number"
                        value="{{ $course->bkash_merchant_number ?? '' }}"
                        class="mt-1 w-full rounded-md border-gray-300  focus:ring-indigo-500 focus:border-indigo-500"
                ...
                <div>
                    ...
                    <select id="status" name="status"
                        class="mt-1 w-full rounded-md border-gray-300  focus:ring-indigo-500 focus:border-indigo-500">
                        @php $selectedStatus = $course->status ?? ''; @endphp
            ...
            <div class="flex items-center gap-3">
                <button
                    class="inline-flex items-center px-4 py-2 rounded-lg bg-green-700 text-white hover:bg-green-600">Update</button>
